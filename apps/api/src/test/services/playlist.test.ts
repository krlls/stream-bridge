...
describe('Playlist service tests', () => {
  ...
  it('Create playlist works', async () => {
  ...
  it('Find playlist by id works', async () => {
    if (isServiceError(currentUser)) {
      throw Error('User not created')
    }
  ...
  it('Find playlist by id works', async () => {
    ...
    const findPlaylist = await playlistService.getAllUserPlaylists({
  ...
  it('Find playlist by external id works', async () => {
    if (isServiceError(currentUser)) {
      throw Error('User not created')
    }
  ...
  it('Export playlist works', async () => {
    if (isServiceError(currentUser)) {
      throw Error('User not created')
    }
  ...
  it('Re-exporting playlists does not create duplicates', async () => {
    ...
    const exportResult = (await playlistService.importPlaylists(exportData)) as any
    const totalPlaylists = (await playlistService.getAllUserPlaylists({
      userId: currentUser.id,
    })) as any
  ...
  it('Remove unused playlists', async () => {
    if (isServiceError(currentUser)) {
      throw Error('User not created')
    }
